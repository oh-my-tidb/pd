...
func (h *hotScheduler) prepareForBalance(cluster opt.Cluster) {
	...
	// TODO: writeLeader should schedule region from `hotLeaders` after
	// hotLeader consider `WriteBytes` and `WriteKeys`.
...
func (bs *balanceSolver) calcProgressiveRank() {
	...
	if bs.rwTy == write && bs.opTy == transferLeader {
		...
		if srcLd.Loads[statistics.KeyDim] >= dstLd.Loads[statistics.KeyDim]+peer.GetLoad(getRegionStatKind(bs.rwTy, statistics.KeyDim)) {
			rank = -1
		}
	} else {
		...
		k := getRegionStatKind(bs.rwTy, statistics.KeyDim)
		keyDecRatio := (dstLd.Loads[statistics.KeyDim] + peer.GetLoad(k)) / getSrcDecRate(srcLd.Loads[statistics.KeyDim], peer.GetLoad(k))
		keyHot := peer.GetLoad(k) >= bs.sche.conf.GetMinHotKeyRate()
		k = getRegionStatKind(bs.rwTy, statistics.ByteDim)
		byteDecRatio := (dstLd.Loads[statistics.ByteDim] + peer.GetLoad(k)) / getSrcDecRate(srcLd.Loads[statistics.ByteDim], peer.GetLoad(k))
		byteHot := peer.GetLoad(k) > bs.sche.conf.GetMinHotByteRate()
