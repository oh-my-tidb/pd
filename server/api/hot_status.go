...
func (h *hotStatusHandler) GetHotStores(w http.ResponseWriter, r *http.Request) {
	...
	for id, loads := range h.GetHotWriteStats() {
		stats.BytesWriteStats[id] = loads[statistics.ByteDim]
		stats.KeysWriteStats[id] = loads[statistics.KeyDim]
	}
	for id, loads := range h.GetHotReadStats() {
		stats.BytesReadStats[id] = loads[statistics.ByteDim]
		stats.KeysReadStats[id] = loads[statistics.KeyDim]
	}
