...
// GetHotStat returns hotspot statistics.
// For test use only.
...
// GetStoresLoads returns load stats of all StoreInfo.
// The result only includes peers that are hot enough.
func (c *RaftCluster) GetStoresLoads() map[uint64][]float64 {
	c.RLock()
	defer c.RUnlock()
	return c.hotStat.GetStoresLoads(c.GetOpts().GetHotRegionCacheHitsThreshold())
}

// HotLeaderStats returns hot region's read stats.
// The result only includes peers that are hot enough.
...
// HotPeerStats returns hot region's write stats.
