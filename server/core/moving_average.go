...
func (f *MaxFilter) Copy() *MaxFilter {
	return &MaxFilter{
		records: append(f.records[:0:0], f.records...),
		size:    f.size,
		count:   f.count,
	}
}

func (f *MaxFilter) Add(n float64) {
	f.records[f.count%f.size] = n
	f.count++
}

func (f *MaxFilter) Get() float64 {
	if f.count == 0 {
		return 0
	}
	records := f.records
	if f.count < f.size {
		records = f.records[:f.count]
	}
...
func (w *WMA) Copy() *WMA {
	return &WMA{
		records: append(w.records[:0:0], w.records...),
		size:    w.size,
		count:   w.count,
	}
}
